...
from src.builders.builder_3_qmpc_deterministic_modeling_and_solving import (
...
RESULTS_QMPC_DIR = REPO / "results" / "qmpc_deterministic"

SETTING_NMPC_YAML = CONFIGS_DIR / "setting_nmpc.yaml"  # reused only for solver options block (optional)
...
# fitted linear model (Δx = M x + N u + O d + m)
FITTED_MATS_NPZ = (REPO / "data" / "for_fitting_qmpc" / "fitting_results" / "fitted_matrices.npz").resolve()

CURRENT_STEP_INFORMATION_JSON = RESULTS_QMPC_DIR / "current_step_information.json"
...
def _load_or_init_current_step_information(defs, keeper: Dict[str, Any]) -> Dict[str, Any]:
    ...
    x = np.asarray([float(x_ini_map[name]) for name in defs.state_names], dtype=float).reshape(len(defs.state_names)).tolist()
...
def _print_step_block(
    *,
    ...
    print()
    print("[1] Solving MPC (QMPC deterministic, Gurobi)...")
    print(
        f"Finished solving (QMPC).  status={solve_status}  objective= {objective: .6f}  solve_time={solve_time_s:.3f}s"
    )
    print()
...
def _load_fitted_matrices(path: Path, defs) -> Dict[str, np.ndarray]:
    if not path.exists():
        raise FileNotFoundError(
            f"Missing fitted matrices at: {path}\n"
            "Run run_experiments/generate_qmpc_dataset.py then run_experiments/fit_qmpc_matrices.py first."
        )
    ...
    m = np.asarray(data["m"], dtype=float).reshape(-1)

    state_names = data.get("state_names", None)
    control_names = data.get("control_names", None)
    disturbance_names = data.get("disturbance_names", None)

    if state_names is None or control_names is None or disturbance_names is None:
        return {"M": M, "N": N, "O": O, "m": m}

    state_names = [str(s) for s in np.asarray(state_names).tolist()]
    control_names = [str(s) for s in np.asarray(control_names).tolist()]
    disturbance_names = [str(s) for s in np.asarray(disturbance_names).tolist()]

    # reorder to match current model defs, if needed
    def _reorder_mat(mat: np.ndarray, rows_from: List[str], cols_from: List[str], rows_to: List[str], cols_to: List[str]) -> np.ndarray:
        rmap = {n: i for i, n in enumerate(rows_from)}
        cmap = {n: i for i, n in enumerate(cols_from)}
        rr = [rmap[n] for n in rows_to]
        cc = [cmap[n] for n in cols_to]
        return mat[np.ix_(rr, cc)]

    def _reorder_vec(vec: np.ndarray, from_names: List[str], to_names: List[str]) -> np.ndarray:
        m0 = {n: i for i, n in enumerate(from_names)}
        return np.asarray([vec[m0[n]] for n in to_names], dtype=float)

    if state_names != defs.state_names:
        M = _reorder_mat(M, state_names, state_names, defs.state_names, defs.state_names)
        m = _reorder_vec(m, state_names, defs.state_names)

    if control_names != defs.control_names:
        N = _reorder_mat(N, state_names, control_names, defs.state_names, defs.control_names)

    if disturbance_names != defs.disturbance_names:
        O = _reorder_mat(O, state_names, disturbance_names, defs.state_names, defs.disturbance_names)
...
def main() -> None:
    ...
    # optional: reuse "setting_nmpc.yaml" just to carry a generic options dict
    solver_options: Dict[str, Any] = {}
    if SETTING_NMPC_YAML.exists():
        try:
            cfg = _read_yaml(SETTING_NMPC_YAML)
            solver_options = dict(cfg.get("gurobi", {}).get("options", {}))
        except Exception:
            solver_options = {}
    ...
    fitted_mats = _load_fitted_matrices(FITTED_MATS_NPZ, defs)
    ...
    for _ in range(remaining):
        ...
        sol = modeling_solving_qmpc_deterministic_problem(model=model, x_ini=x_ini, d_pred_h=d_pred_h, step_index=i,
                                                          mpc_global_parameters=mpc, fitted_mats=fitted_mats,
                                                          solver_options=solver_options, K_eff=K_eff)
        solve_time_s = time.perf_counter() - t_solve0

        u0 = sol.U_opt[0].copy() if sol.U_opt.size else np.zeros((len(defs.control_names),), dtype=float)
